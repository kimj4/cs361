 ...
 *@author Sherri Goings - added sel (determines which selection algorithm to use), and seed (value to seed random number generator)
 *to rules settings
 */
public class RulesDialog extends JPanel
{
    //Swing Components
    JSpinner maxGenSp, numPlayersSp,iterationsSp, mutateSp, crossoverSp,  temptationSp, rewardSp, suckerSp, punishSp, selSp, seedSp, sParamSp, evalSp;
    JLabel maxGenLbl, numPlayersLbl,mutateLbl, crossoverLbl, iterationsLbl, temptationLbl, rewardLbl, suckerLbl, punishLbl, selLbl, seedLbl, sParamLbl, evalLbl;
    JButton defaultBtn, saveBtn, restoreBtn;
    JDialog rulesDlg;
	
    //Rule settings
    private int gen, plyrs, itr, t, r, s, p, sel, seed, sParam, eval;
    private double m, c;
	...
	if(exist)
		...
		if(props.getProperty("c") != null)
		    ...
		    c = .95;
		if(props.getProperty("sel") != null)
		    sel = Integer.parseInt(props.getProperty("sel"));
		else
		    sel = 0;
		if(props.getProperty("seed") != null)
		    ...
		    seed = -1;
		if(props.getProperty("sParam") != null)
		    sParam = Integer.parseInt(props.getProperty("sParam"));
		else
		    sParam = 1;
		if(props.getProperty("eval") != null)
		    eval = Integer.parseInt(props.getProperty("eval"));
		else
		    eval = 0;
	    }
	else
	...
	suckerLbl = new JLabel("TooTrusting Payoff:", SwingConstants.LEFT);
	...
	model = new SpinnerNumberModel(sel, 0, 10, 1); 
	selSp  = new JSpinner(model);
	selLbl = new JLabel("Selection:", SwingConstants.LEFT);

	model = new SpinnerNumberModel(sParam, 0, 1000, 1); 
	sParamSp = new JSpinner(model);
	sParamLbl = new JLabel("Selection Parameter:", SwingConstants.LEFT);

	model = new SpinnerNumberModel(eval, 0, 100, 1); 
	evalSp = new JSpinner(model);
	evalLbl = new JLabel("Evaluation Method:", SwingConstants.LEFT);

	model = new SpinnerNumberModel(seed, -1, 9999999, 1); 
	seedSp = new JSpinner(model);
	seedLbl = new JLabel("Random Number Seed:", SwingConstants.LEFT);
	...
	defaultBtn.addActionListener(new ActionListener() {
		public void actionPerformed(ActionEvent e) 
		{
		    ...
		    selSp.setValue(new Integer(0));
		    seedSp.setValue(new Integer(-1));
		    sParamSp.setValue(new Integer(1));
		    evalSp.setValue(new Integer(0));
		}
	    });
	...
	restoreBtn.addActionListener(new ActionListener() {
		public void actionPerformed(ActionEvent e) 
		{
		    ...
		    selSp.setValue(new Integer(sel));
		    seedSp.setValue(new Integer(seed));
		    sParamSp.setValue(new Integer(sParam));
		    sParamSp.setValue(new Integer(eval));
		    rulesDlg.setVisible(false);
		}
	    });
	...
	return new Rules(itr, t, s, r, p, m, c, sel, sParam, eval, seed);
    }
	...
	sel =  ((Integer)selSp.getValue()).intValue();
	sParam =  ((Integer)sParamSp.getValue()).intValue();
	eval =  ((Integer)evalSp.getValue()).intValue();
	seed =  ((Integer)seedSp.getValue()).intValue();

        mf.setRules(new Rules(itr, t,s,r,p, m,c, gen, plyrs, sel, sParam, eval, seed));
	...
	props.setProperty("sel", Integer.toString(sel));
	props.setProperty("sParam", Integer.toString(sParam));
	props.setProperty("eval", Integer.toString(eval));
	props.setProperty("seed", Integer.toString(seed));
	try
	    {
